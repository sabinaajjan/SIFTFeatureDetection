...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    image_bw = torch.tensor(image_bw).unsqueeze(0).unsqueeze(0)  
    x_kernel = torch.tensor(SOBEL_X_KERNEL).unsqueeze(0)  
    y_kernel = torch.tensor(SOBEL_Y_KERNEL).unsqueeze(0)  

    x_kernel = x_kernel.reshape((1, 1, 3, 3))
    y_kernel = y_kernel.reshape((1, 1, 3, 3))

    Ix = torch.conv2d(image_bw, weight=x_kernel, padding=1)
    Iy = torch.conv2d(image_bw, weight=y_kernel, padding=1)

    Ix = Ix.squeeze(0).squeeze(0).numpy()
    Iy = Iy.squeeze(0).squeeze(0).numpy()
...
def compute_harris_response_map(
    ...
    M = np.bmat([[Sxx, Sxy], [Sxy, Syy]])
    determinant = Sxx * Syy - Sxy**2
    t = Sxx + Syy
    R = determinant - alpha * (t ** 2)
...
def maxpool_numpy(R: np.ndarray, ksize: int) -> np.ndarray:
    ...
    whats happening in nms_maxpool_pytorch.
...
def nms_maxpool_pytorch(
    ...
    """
    tensor_R = torch.tensor(R).unsqueeze(0).unsqueeze(0)
    median_value = torch.median(tensor_R)
    thresholded = torch.where(tensor_R < median_value, torch.tensor(0, dtype=tensor_R.dtype), tensor_R)
    m = nn.MaxPool2d(ksize, stride=1, padding=ksize//2)
    output = m(thresholded)
    binary_image = (tensor_R == output).float()
    not_confidences = (binary_image * tensor_R).flatten()
    confidences, indices = torch.topk(not_confidences, k)
    y, x = np.unravel_index(indices.numpy(), R.shape)
...
def get_harris_interest_points(
    ...
    #raise NotImplementedError('`get_harris_interest_points` function in ' +
       # '`part1_harris_corner.py` needs to be implemented')
    map = compute_harris_response_map(image_bw)
    
    map_normalized = map - np.min(map)
    map_normalized /= np.max(map_normalized)
    kernel_size = 7
    x, y, c = nms_maxpool_pytorch(map_normalized, k, kernel_size)
