...
def compute_feature_distances(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    ss1 = np.sum(np.square(features1), axis=1).reshape(-1, 1)
    ss3 = np.sum(np.square(features2), axis=1).reshape(1, -1)
    dot_products = -2 * np.dot(features1, features2.T) 
    dists = np.sqrt(np.maximum(ss1 + dot_products + ss3, 0))
...
def match_features_ratio_test(
    ...
    distances_sorted = np.argsort(dists, axis=1)
    
    nearest = distances_sorted[:, 0]
    second_nearest = distances_sorted[:, 1]
    
    fractions = dists[np.arange(dists.shape[0]), nearest] / dists[np.arange(dists.shape[0]), second_nearest]
    
    ratio_threshold = 0.8
    valid_matches = fractions < ratio_threshold
    
    matches_indices = np.where(valid_matches)[0]
    
    matches = np.column_stack((matches_indices, nearest[valid_matches]))
    confidences = 1 - fractions[valid_matches]
